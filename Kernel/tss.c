...
tss_t tss_create_user_task(paddr_t code_start) {
  ...
  // Asigno el valor inicial de la pila de la tarea
  ...
  paddr_t stackKernel = mmu_next_free_kernel_page();    //ya mapeado por identidad
  vaddr_t stack0 = (vaddr_t)stackKernel;
