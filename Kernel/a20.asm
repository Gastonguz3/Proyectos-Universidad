...
A20_disable:
    pushf
    pusha
    print_text_rm disable_msg, disable_len, 0x07, 0, 0
    call    A20_wait
    mov     al,0xAD
    out     0x64,al
    call    A20_wait
    ...
    out     0x64,al
    call    A20_wait
    print_text_rm ok_msg, ok_len, 0x0A, 0xFFFF, 23
    popa
    popf
    ret
...
A20_enable:
    pushf
    pusha
    print_text_rm enable_msg, enable_len, 0x07, 0, 0
    call    A20_wait
    mov     al,0xAD
    out     0x64,al
    call    A20_wait
...
A20_check:
    ...
    push di
    push si
    print_text_rm check_msg, check_len, 0x07, 0, 0
    xor ax, ax ; ax = 0
    mov fs, ax
    not ax ; ax = 0xFFFF
    mov gs, ax
    ...
    jmp .sigue
    .falla:
        print_text_rm fail_msg, fail_len, 0x0C, 0xFFFF, 23
    .sigue:
    pop ax
    mov byte [gs:si], al
    pop ax
