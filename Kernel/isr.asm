...
; Rutina de atencion del page fault
global _isr14
extern page_fault_handler
_isr14:
		pushad
    mov edi, [esp + 32]     ;levanto el codigo de error
    and edi, 1
    jnz false
    mov eax, cr2
    push eax
...
global _isr32
_isr32:
    pushad                ; Guarda los registros de proposito general
    ...
    call sched_next_task  ; Consigue el selector de la proxima tarea
    cmp ax, 0     
    je .fin

    str bx                ; Guarda en el registro cx el valor del registro TR
    cmp ax, bx            ; Compara el selector de la siguiente tarea con el actual 
    je .fin               ; Si la próxima tarea es la actual no cambio de tarea

    mov word [sched_task_selector], ax  ; Sino se carga en la posicion de memoria de sched_task_selector el nuevo selector
    jmp far [sched_task_offset]         ; Se hace jump far a la nueva tarea

    .fin:
    call tasks_tick             ; Actualizamos las estructuras compartidas ante el tick del reloj
    call tasks_screen_update    ; Actualizamos la "interfaz" del sistema en pantalla

    popad                 ; Recupera los registros de proposito general
    iret                  ; Vuelve a la rutina que la llamo restaurando el EIP
...
_isr33:
    ...
    call pic_finish1      ; Le decimos al PIC que vamos a atender la interrupción
    ; Leemos la tecla desde el teclado y la procesamos
    in al, 0x60
    push eax
